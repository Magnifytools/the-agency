...
export function DeliveryReceipts({ sourceKind, sourceId }: { sourceKind: "daily" | "digest"; sourceId: number }) {
  const { user } = useAuth()
  const client = useQueryClient()
  const [review, setReview] = useState<{ receipt: DeliveryReceipt; key: string } | null>(null)
  const [acknowledged, setAcknowledged] = useState(false)
  const key = ["deliveries", user?.id, sourceKind, sourceId]
  const query = useQuery({
    queryKey: key,
    queryFn: () => deliveriesApi.list(sourceKind, sourceId),
    enabled: !!user,
  ...
  useEffect(() => {
    if (terminalReceipts && sourceKind === "daily") {
      client.invalidateQueries({ queryKey: ["dailys"] })
      client.invalidateQueries({ queryKey: ["daily-today"] })
    }
  }, [terminalReceipts, sourceKind, client])
