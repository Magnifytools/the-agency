...
const statusConfig = {
  ...
  no_capacity: { label: "Sin capacidad configurada", color: "bg-muted-foreground/40", text: "text-muted-foreground" },
}
  ...
  return (
    <div className="space-y-4">
      {data.map((member) => {
        ...
        const pct = member.load_percent == null ? 0 : Math.min(member.load_percent, 100)
        return (
          <div key={member.user_id}>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-bold ${cfg.text}`}>{member.load_percent == null ? "Sin capacidad" : `${member.load_percent}%`}</span>
              </div>
            </div>
