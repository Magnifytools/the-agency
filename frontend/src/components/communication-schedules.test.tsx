...
it("shows opt-in, paused status and a real extension update link", async () => {
  show()
  expect(await screen.findByText(/El planificador está pausado/)).toBeInTheDocument()
  ...
  expect(screen.getByText(/DM personal de Discord no está disponible/)).toBeInTheDocument()
})
it("saves actual channels and advance notice, preserving draft after an error", async () => {
  ...
  show()
  fireEvent.click(await screen.findByRole("checkbox", { name: "Activar reuniones" }))
...
it("reloads a changed revision only when the user discards their draft", async () => {
  ...
  show()
  fireEvent.click(await screen.findByRole("button", { name: "Guardar reuniones" }))
  await screen.findByRole("alert")
