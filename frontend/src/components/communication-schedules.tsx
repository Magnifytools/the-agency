...
type Kind = "morning" | "team_morning" | "evening" | "meeting" | "weekly"
...
const titles: Record<Kind, string> = { morning: "Plan personal de la mañana", team_morning: "Plan matinal del equipo", evening: "Resumen del día", meeting: "Reuniones", weekly: "Informe semanal del equipo" }
...
const channels: Record<Kind, string[]> = { morning: ["in_app", "team_webhook"], team_morning: ["team_webhook"], evening: ["in_app", "team_webhook"], meeting: ["in_app", "extension"], weekly: ["owner_dm"] }
...
function PolicyEditor({ policy, refresh }: { policy: Policy; refresh: () => void }) {
  ...
  return <details className="rounded-xl border p-4" open={policy.state === "blocked" ? true : undefined}>
    ...
    {policy.kind === "morning" && <p className="text-sm text-muted-foreground">Este aviso es personal: recoge tus tareas de Hoy, arrastres y tareas sin planificar. Si quieres un único mensaje compartido con el trabajo de todo el equipo, un administrador puede configurar el plan matinal del equipo.</p>}
    {policy.kind === "team_morning" && <p className="text-sm text-muted-foreground">Un administrador publica un único mensaje en el Discord compartido a las 08:00 (Europe/Madrid). Agrupa el trabajo por persona y muestra las tareas sin responsable una sola vez. Activarlo no activa ni cambia los avisos personales de nadie.</p>}
...
export function CommunicationSchedules() {
  ...
  return <section id="notifications" className="bg-card border border-border rounded-2xl p-6 space-y-4 scroll-mt-8">
    ...
    {catalog.data && <>
      ...
      <div className="grid gap-4 lg:grid-cols-2">{catalog.data.policies.filter(policy => policy.kind !== "team_morning" || isAdmin).map(policy => <PolicyEditor key={`${user?.id}:${policy.kind}`} policy={policy} refresh={refresh} />)}</div>
    </>}
