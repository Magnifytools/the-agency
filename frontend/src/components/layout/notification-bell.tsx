...
export function NotificationBell({ onNavigate, showLabel = false }: { onNavigate?: () => void; showLabel?: boolean } = {}) {
  const { user } = useAuth()
  return user ? <RecipientBell key={user.id} userId={user.id} onNavigate={onNavigate} showLabel={showLabel} /> : null
}

function RecipientBell({ userId, onNavigate, showLabel }: { userId: number; onNavigate?: () => void; showLabel: boolean }) {
  ...
  return (
    <div className="relative" ref={panelRef}>
      <button ref={triggerRef} type="button" aria-label="Alertas y actividad" aria-expanded={open} aria-controls={open ? panelId : undefined}
        title="Ver asuntos que requieren atención y actividad reciente"
        onClick={toggle} className={`relative rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground ${showLabel ? "flex min-h-11 w-full flex-col items-center justify-center gap-1 text-xs" : ""}`}>
        <Bell className="h-4 w-4" aria-hidden="true" />
        {showLabel && <span aria-hidden="true">Alertas</span>}
      ...
      {open && <div id={panelId} role="dialog" aria-label="Alertas y actividad" style={position} className={`fixed ${showLabel ? "z-[70]" : "z-50"} max-h-[min(32rem,calc(100dvh-8rem))] w-96 max-w-[calc(100vw-2rem)] overflow-auto rounded-xl border border-border bg-card shadow-xl`}>
        ...
        <p className="border-b border-border px-3 py-2 text-xs text-muted-foreground">
          {view === "incidents" ? "Asuntos que requieren atención. Abrirlos no los resuelve." : "Novedades recientes; puedes marcarlas como leídas."}
        </p>
