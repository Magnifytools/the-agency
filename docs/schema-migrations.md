...
P15 appends `schema_versions/inbox_recovery_v2.json`: two nullable retry fields on `inbox_notes` and a partial index for pending work ordered by its effective next attempt. It preserves note text, status, associations, suggestions and timestamps. Verification checks column shape and the complete index definition. The P12 and P14 artifacts remain unchanged.
...
The first P15 release failed and rolled back before traffic: its v1 index mixed a new `timestamp` column with legacy `timestamptz` timestamps, which PostgreSQL cannot index immutably. The v1 files remain unchanged as failed-release evidence but are not in the current plan. The verified production ledger contains only P12 + P14. V2 orders the queue by `COALESCE(classification_next_attempt_at, TIMESTAMP 'epoch'), id`; new captures and retries store their UTC attempt time, while legacy pending rows drain first. Compare-and-set uses PostgreSQL `extract(epoch FROM updated_at)` without rebinding aware timestamps through the naive ORM type. Upgrade and race tests cover both timestamp variants and different session time zones. An installation that actually applied v1 must receive a separately reviewed adoption plan; never remove its ledger entry.
