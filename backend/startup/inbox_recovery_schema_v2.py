...
_BYTES = (Path(__file__).with_name("schema_versions") / "inbox_recovery_v2.json").read_bytes()
...
async def verify(conn):
    ...
    if not index or not (
        ...
        and index["first_key"] == "COALESCE(classification_next_attempt_at, '1970-01-01 00:00:00'::timestamp without time zone)"
